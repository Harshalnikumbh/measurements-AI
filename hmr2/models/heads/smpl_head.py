...
class SMPLTransformerDecoderHead(nn.Module):
    ...
    def forward(self, x, **kwargs):
        ...
        for _ in range(self.cfg.MODEL.SMPL_HEAD.get('IEF_ITERS', 1)):
            ...
            token_out = self.transformer(token, context=x).squeeze(1)
        ...
        pred_smpl_params_list = {
            'body_pose': torch.cat(
                [
                    joint_conversion_fn(pbp)
                    .view(batch_size, -1, 3, 3)[:, 1:, :, :]
        ...
        final_pose = joint_conversion_fn(pred_body_pose).view(
            batch_size,
        ...
        pred_smpl_params = {
            'global_orient': final_pose[:, [0]],
            'body_pose': final_pose[:, 1:],
            'betas': pred_betas
        }
